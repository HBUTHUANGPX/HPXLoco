...
from .pai_config import PaiCfg


class Pai(LeggedRobot):
    def __init__(self, cfg: PaiCfg, sim_params, physics_engine, sim_device, headless):
        self.cfg = cfg
    ...
    def step(self, actions):
        ...
        self.delayed_actions = (
            self.actions.clone()
            .view(self.num_envs, 1, self.num_actions)
            .repeat(1, self.cfg.control.decimation, 1)
        )  # 将当前的动作 self.actions 克隆并调整形状，然后重复多次以创建一个延迟动作的张量 self.delayed_actions
        delay_steps = torch.randint(
            0, self.cfg.control.decimation, (self.num_envs, 1), device=self.device
        )  # 生成一个随机延迟步数的张量 delay_steps，每个环境的延迟步数在 [0, self.cfg.control.decimation) 范围内
        if self.cfg.domain_rand.delay:  # 根据生成的随机延迟步数，对动作进行延迟处理。
            for i in range(self.cfg.control.decimation):  # 遍历每个控制周期的分割数。
        ...
        for _ in range(self.cfg.control.decimation):
            self.torques = self._compute_torques(self.delayed_actions[:,_]).view(
                self.torques.shape
            )
    ...
    def post_physics_step(self):
        ...
        self.feet_pos = self.rigid_body_states.view(self.num_envs, self.num_bodies, 13)[
            :, self.feet_indices, 0:3
        ]
        self.feet_vel = self.rigid_body_states.view(self.num_envs, self.num_bodies, 13)[
    ...
    def update_command_curriculum(self, env_ids):
        ...
        if (
            torch.mean(self.episode_sums["tracking_lin_vel"][low_vel_env_ids])
            / self.max_episode_length
            > 0.8 * self.reward_scales["tracking_lin_vel"]
        ) and (
            torch.mean(self.episode_sums["tracking_lin_vel"][high_vel_env_ids])
            / self.max_episode_length
            > 0.8 * self.reward_scales["tracking_lin_vel"]
        ):
    ...
    def _create_envs(self):
        ...
        print("len(feet_names):",len(feet_names))
    ...
    def _post_physics_step_callback(self):
        ...
        if self.cfg.commands.heading_command:
            ...
            self.commands[:, 2] = torch.clip(
                0.5 * wrap_to_pi(self.commands[:, 3] - heading), -2.0, 2.0
            )
    ...
    def _compute_torques(self, actions):
        ...
        # print("self.cfg.control.action_scale:",(self.cfg.control.action_scale.size()))
    ...
    def _get_noise_scale_vec(self, cfg):
        ...
        noise_vec[(9 + 2 * self.num_actions) : (9 + 3 * self.num_actions)] = (
    ...
    def _get_base_heights(self, env_ids=None):
        ...
        base_height = torch.mean(
            self.root_states[:, 2].unsqueeze(1) - base_height, dim=1
        )
