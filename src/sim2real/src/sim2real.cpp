...
void Robot::read(){
    ...
    for(int i=0; i<cfg.num_actions; i++){
        q[i] = q[i] * cfg.motor_direction[i] - params.init_dof_pos[0][i].item<double>();
    ...
    // base_ang_vel<<  0.1*yesenceIMU_.angular_velocity.x + 0.9*base_ang_vel[0],
    //                 0.1*yesenceIMU_.angular_velocity.y + 0.9*base_ang_vel[1],
    //                 0.1*yesenceIMU_.angular_velocity.z + 0.9*base_ang_vel[2];
...
void Robot::write(){
    ...
    for(int i=0; i<cfg.num_actions; i++){
        ...
        dynamic_offset_knee = 0.03;
        dynamic_offset_ankle = 0.0;

        if ( (i==2) ){
            target_q[i]-=dynamic_offset_hip;
        }
        else if ( (i==8) ){
            target_q[i]-=dynamic_offset_hip;
        }
        else if ( (i==3) ){
            target_q[i]-=dynamic_offset_knee;
        }
        else if ( (i==9) ){
            target_q[i]-=dynamic_offset_knee;
        }
        else if ( (i==4)||(i==11) ){
            target_q[i]-=dynamic_offset_ankle;
        }
        else if ( (i==5)||(i==10) ){
            target_q[i]+=dynamic_offset_ankle;
        }
...
void Robot::update_obs(){
    ...
    obs[0] = std::sin(2 * M_PI * count_lowlevel * cfg.dt / 0.5);
    obs[1] = std::cos(2 * M_PI * count_lowlevel * cfg.dt / 0.5);
    ...
    obs.segment(41, 3) = base_ang_vel * 0.5;
...
void Robot::joy_callback(const sensor_msgs::Joy::ConstPtr& msg)
{
    ...
    switch(state)
    {
        case  sys_state::close:
            if(msg->buttons[9]==1) state= sys_state::init;
            if(msg->buttons[5]==1) state= sys_state::exit;

            if(msg->buttons[3]==1) dynamic_offset_hip+=0.005;
            if(msg->buttons[0]==1) dynamic_offset_hip-=0.005;
            if(msg->buttons[1]==1) dynamic_offset_knee+=0.005;
            if(msg->buttons[2]==1) dynamic_offset_knee-=0.005;
            if(msg->buttons[7]==1) dynamic_offset_ankle+=0.005;
            if(msg->buttons[6]==1) dynamic_offset_ankle-=0.005;
        break;
        case sys_state::init:
            if(msg->buttons[4]==1)
            {
        ...
        case sys_state::exec:
            ...
            if((double)msg->axes[1] >= 0.0)
            {
                if((double)msg->axes[2] < -0.1) cmd.vx = ((double)msg->axes[1])*0.15;
                else cmd.vx = ((double)msg->axes[1])*0.05;
            }
            else if((double)msg->axes[1] < -0.0)
            {
                if((double)msg->axes[2] < -0.1) cmd.vx = ((double)msg->axes[1])*0.3;
                else cmd.vx = ((double)msg->axes[1])*0.2;
            }

            if((double)msg->axes[0] >= 0.0) cmd.vy = ((double)msg->axes[0])*0.2;
            else if((double)msg->axes[0] < -0.0) cmd.vy = ((double)msg->axes[0])*0.2;

            if((double)msg->axes[3] >= 0.0) cmd.dyaw = ((double)msg->axes[3])*0.6;
            else if((double)msg->axes[3] < -0.0) cmd.dyaw = ((double)msg->axes[3])*0.6;
            ...
            //     else cmd.vx = ((double)msg->axes[1])*0.05;
            // }
            // else if((double)msg->axes[1] < -0.0) cmd.vx = ((double)msg->axes[1])*0.3;

            // if((double)msg->axes[0] >= 0.0) cmd.vy = ((double)msg->axes[0])*0.3;
            // else if((double)msg->axes[0] < -0.0) cmd.vy = ((double)msg->axes[0])*0.1;
            ...
            // ROS_INFO("%f,%f,%f",(double)msg->axes[0],(double)msg->axes[1],(double)msg->axes[3]);
            if(msg->buttons[3]==1) dynamic_offset_hip+=0.005;
            if(msg->buttons[0]==1) dynamic_offset_hip-=0.005;
            if(msg->buttons[1]==1) dynamic_offset_knee+=0.005;
            if(msg->buttons[2]==1) dynamic_offset_knee-=0.005;
...
{
    ros::Rate r(100);
    Robot rbt("/home/hpx/HPXLoco/src/sim2real/policy/2024_0717_HIM_6100_him_policy.pt");
    // Robot rbt("/home/orangepi/rrrl_ws/src/policy/policy_1_height_0.03.pt");
    while(ros::ok())
    {
